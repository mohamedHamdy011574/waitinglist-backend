...
@section('content')
  <section class="content-header">
    ...
    <ol class="breadcrumb">
      ...
      <li>
        <a href="{{route('restaurants.index')}}">{{trans('concerns.singular')}}</a>
      </li>
      <li class="active">
        {{ trans('concerns.edit') }}
      </li>
    ...
    <div class="row">
      <div class="col-md-12">
        <div class="box">
          ...
          <div class="box-body">
            <form method="POST" action="{{route('concerns.update', $concern->id)}}" accept-charset="UTF-8" enctype="multipart/form-data">
              @csrf
                      ...
                      @foreach(config('app.locales') as $lk=>$lv)
                        <div role="tabpanel" class="tab-pane @if($lk=='en') active @endif" id="abc_{{$lk}}">
                          <div class="form-group">
                            ...
                            <input class="form-control"  placeholder="{{trans('concerns.concern')}}" name="concern:{{$lk}}" id="concern:{{$lk}}" type="text" value="{{$concern->translate($lk)->concern}}" 
                      ...
                      <div class="col-md-6">
                        <div class="form-group">
                          ...
                          <select class="form-control" name="status" id="status" required>
                            <option value="active" 
                              @if($concern->status == 'active') selected @endif>
                              {{trans('common.active')}}
                            </option>
                            <option value="inactive" 
                              @if($concern->status == 'inactive') selected @endif>
