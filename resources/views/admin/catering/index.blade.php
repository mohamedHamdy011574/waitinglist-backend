...
@section('content')
  <section class="content-header">
    <h1>
      {{trans('catering.plural')}}
    </h1>
    <ol class="breadcrumb">
      <li><a href="{{route('home')}}"><i class="fa fa-dashboard"></i> {{trans('common.home')}}</a></li>
      <li><a href="{{route('catering.index')}}">{{trans('catering.plural')}}</a></li>
    </ol>
  </section>
      ...
      <div class="col-xs-12">
        <div class="box">
          <div class="box-header">
            <h3 class="box-title">{{ trans('catering.title') }}</h3>
            @can('catering-create')
            <h3 class="box-title pull-right"><a href="{{route('catering.create')}}" class="btn btn-danger pull-right">{{trans('catering.add_new')}}</a></h3>
            @endcan
          </div>
          <div class="box-body">
            ...
            <!-- Cuisines Filter -->
            <div class="col-xs-4">
              <select class="form-control select2" id="cuisineschooser" name="cuisines[]" multiple data-placeholder="{{trans('catering.select_cuisines')}}" required>
            ...
            <!-- Cuisines Filter -->
            <table id="catering_table" class="table table-bordered table-hover">
              <thead>
                <tr>
                  <th>{{trans('common.id')}}</th>
                  <th>{{trans('catering.name')}}</th>
                  <th>{{trans('catering.banner')}}</th>
                  <th>{{trans('catering.cuisines')}}</th>
                  <th>{{trans('catering.price')}} {{$currency}}</th>
                  <th>{{trans('catering.registration_date')}}</th>
...
<script type="text/javascript">
  $(document).on('change','.catering_status',function(){
      var status = $(this).val();
      var id = $(this).attr('id');
      var delay = 500;
      var element = $(this);
...
<script type="text/javascript">
  $(document).ready(function(){
        ...
        columns: [
           ...
           { data: 'status',
              mRender : function(data, type, row) {
                   ...
                   return '<select class="catering_status form-control" id="'+row["id"]+'"><option value="active"'+type+'>'+'{{trans("common.active")}}'+'</option><option value="inactive"'+data+'>'+'{{trans("common.inactive")}}'+'</option></select><span class="loading" style="visibility: hidden;"><i class="fa fa-spinner fa-spin fa-1x fa-fw"></i><span class="sr-only">Loading...</span></span>';
                } 
            },
           { 
              mRender : function(data, type, row) {
    ...
    $('#cuisineFiltee').click(function() {
      $('#catering_table').DataTable().destroy();
