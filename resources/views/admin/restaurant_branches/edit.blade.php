...
@section('content')
  <section class="content-header">
    <h1>
      {{ trans('restaurant_branches.edit') }}
    </h1>
    <ol class="breadcrumb">
      <li>
        <a href="{{route('home')}}">
                ...
                <div class="row">
                  <div class="col-md-6">
                      ...
                      @foreach(config('app.locales') as $lk=>$lv)
                        <div role="tabpanel" class="tab-pane @if($lk=='en') active @endif" id="abc_{{$lk}}">
                          <div class="form-group">
                            ...
                            <input class="form-control"  placeholder="{{trans('restaurant_branches.name')}}" name="name:{{$lk}}" id="name:{{$lk}}" type="text" value="{{$restaurant_branch->translate($lk)->name}}" required>
                            <strong class="help-block"></strong>
                          </div>
                          <div class="form-group">
                            <label for="address:{{$lk}}" class="content-label">{{trans('restaurant_branches.address')}}</label>
                            <textarea class="form-control" rows="5" placeholder="{{trans('restaurant_branches.address')}}" name="address:{{$lk}}" id="address:{{$lk}}" required>{{$restaurant_branch->translate($lk)->address}}</textarea>
                            <strong class="help-block"></strong>
                          </div>
                        </div>    
                    ...
                    <div class="form-group">
                      ...
                      <select class="form-control select2" id="restaurant" name="restaurant_id" data-placeholder="{{trans('restaurant_branches.select_restaurants')}}" required>
                        @foreach($restaurants as $restaurant)
                        <option value="{{$restaurant->id}}"
                          {{ ($restaurant_branch->restaurant_id == $restaurant->id) ? 'selected':'' }}
                          >{{$restaurant->name}}
                        </option>
                        @endforeach
                      </select>
                  ...
                  <div class="col-md-6">
                    <div class="form-group">
                        <label for="total_seats" class="content-label">{{trans('restaurant_branches.total_seats')}}</label>
                          <input class="form-control"  placeholder="{{trans('restaurant_branches.total_seats')}}" name="total_seats" id="total_seats" type="number" value="{{$restaurant_branch->total_seats}}" required>
                        <strong class="help-block"></strong>
                    </div>

                    <!-- <div class="form-group">
                        <label for="available_seats" class="content-label">{{trans('restaurant_branches.available_seats')}}</label>
                          <input class="form-control"  placeholder="{{trans('restaurant_branches.available_seats')}}" name="available_seats" id="available_seats" type="number" value="{{$restaurant_branch->available_seats}}" required>
                        <strong class="help-block"></strong>
                    </div> -->
                    ...
                    <div class="form-group">
                      ...
                      <select class="form-control select2" id="state" name="state_id" data-placeholder="{{trans('restaurant_branches.select_state')}}" required>
                        ...
                        <option value="{{$state->id}}"
                          {{ ($restaurant_branch->state_id == $state->id) ? 'selected':'' }}
                          >{{$state->name}}
                        </option>
                        @endforeach
                      </select>
                    ...
                    <div class="form-group">
                      ...
                      <select class="form-control" name="status" id="status" required>
                        <option value="active" 
                          @if($restaurant_branch->status == 'active') selected @endif>
                          {{trans('common.active')}}
                        </option>
                        <option value="inactive" 
                          @if($restaurant_branch->status == 'inactive') selected @endif>
