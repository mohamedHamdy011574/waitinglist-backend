      ...
      <div class="col-xs-12">
        <div class="box">
          <div class="box-header">
            <h3 class="box-title">{{ trans('seating_area.title') }}</h3>
            @can('seating-area-create')
            <h3 class="box-title pull-right"><a href="{{route('seating_area.create')}}" class="btn btn-danger pull-right">{{trans('seating_area.add_new')}}</a></h3>
            @endcan
          </div>
          <div class="box-body">
            <table id="seating_area_table" class="table table-bordered table-hover">
              <thead>
                <tr>
                  <th>{{trans('common.id')}}</th>
                  <th>{{trans('seating_area.name')}}</th>
                  @can('seating-area-status')
...
<script type="text/javascript">
  $(document).on('change','.seating_area_status',function(){
      var status = $(this).val();
      var id = $(this).attr('id');
      var delay = 500;
      var element = $(this);
      ...
      columns: [
         ...
         { data: 'status',
            mRender : function(data, type, row) {
                 ...
                 return '<select class="seating_area_status form-control" id="'+row["id"]+'"><option value="active"'+type+'>'+'{{trans("common.active")}}'+'</option><option value="inactive"'+data+'>'+'{{trans("common.inactive")}}'+'</option></select><span class="loading" style="visibility: hidden;"><i class="fa fa-spinner fa-spin fa-1x fa-fw"></i><span class="sr-only">Loading...</span></span>';
              } 
          },
          @endcan
         { 
            mRender : function(data, type, row) {
                 return '@can("seating-area-edit")<a class="btn" href="'+row["edit"]+'"><i class="fa fa-edit"></i></a>@endcan<a class="btn" href="'+row["show"]+'"><i class="fa fa-eye"></i></a>@can("seating-area-delete")<form action="'+row["delete"]+'" method="post"><button class="btn" type="submit" onclick=" return delete_alert()"><i class="fa fa-trash"></i></button>@method("delete")@csrf</form>@endcan';  
