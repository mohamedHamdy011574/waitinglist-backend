...
@section('content')
  <section class="content-header">
    <h1>
      {{trans('food_truck_menus.plural')}}
    </h1>
    <ol class="breadcrumb">
      <li><a href="{{route('home')}}"><i class="fa fa-dashboard"></i> {{trans('common.home')}}</a></li>
      <li><a href="{{route('food_truck_menus.index')}}">{{trans('food_truck_menus.plural')}}</a></li>
    </ol>
  </section>
      ...
      <div class="col-xs-12">
        <div class="box">
          <div class="box-header">
            <h3 class="box-title">{{ trans('food_truck_menus.title') }}</h3>
            @can('menu-create')
            <h3 class="box-title pull-right"><a href="{{route('food_truck_menus.create')}}" class="btn btn-danger pull-right">{{trans('food_truck_menus.add_new')}}</a></h3>
            @endcan
          </div>
          <div class="box-body">
          
              <table id="food_truck_menu_table" class="table table-bordered table-hover">
              <thead>
                <tr>
                  <th>{{trans('common.id')}}</th>
                  <th>{{trans('food_truck_menus.name')}}</th>
                  <th>{{trans('food_truck_menus.category')}}</th>
                  <th>{{trans('food_truck_menus.price')}} {{$currency}}</th>
...
<script type="text/javascript">
  $(document).on('change','.food_truck_menu_status',function(){
      var status = $(this).val();
      var id = $(this).attr('id');
      var delay = 500;
      var element = $(this);
      $.ajax({
          type:'post',
          url: "{{route('food_truck_menu_status')}}",
          data: {
                  "status": status, 
                  "id" : id,  
                  "_token": "{{ csrf_token() }}"
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    function fill_datatable() {
      $('#food_truck_menu_table').DataTable({
        dom: 'Blfrtip',
        ...
        columns: [
           ...
           { data: 'status',
              mRender : function(data, type, row) {
                   ...
                   return '<select class="food_truck_menu_status form-control" id="'+row["id"]+'"><option value="active"'+type+'>'+'{{trans("common.active")}}'+'</option><option value="inactive"'+data+'>'+'{{trans("common.inactive")}}'+'</option></select><span class="loading" style="visibility: hidden;"><i class="fa fa-spinner fa-spin fa-1x fa-fw"></i><span class="sr-only">'+'{{trans("common.loading")}}'+'</span></span>';
                } 
            },
           { 
              mRender : function(data, type, row) {
