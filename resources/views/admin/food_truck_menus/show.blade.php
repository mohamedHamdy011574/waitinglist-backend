          ...
          <div class="box-body">
            <form method="POST" id="message_templateForm" action="{{route('food_truck_menus.update', $food_truck_menu->id)}}" accept-charset="UTF-8" enctype="multipart/form-data">
                      ...
                      @foreach(config('app.locales') as $lk=>$lv)
                        <div role="tabpanel" class="tab-pane @if($lk=='en') active @endif" id="abc_{{$lk}}">
                          <div class="form-group">
                            ...
                            <p class="details">{{$food_truck_menu->translate($lk)->name}}
                            </p> 
                          </div>
                          <div class="form-group">
                            <label for="description:{{$lk}}" class="content-label">{{trans('food_truck_menus.description')}}</label>
                            <div class="details"> {!!$food_truck_menu->translate($lk)->description!!}
                            </div>
                          </div>
                        </div>    
                  ...
                  <div class="col-md-6">
                    <div class="form-group">
                      ...
                      @foreach($menu_categories as $mc)
                        @if($food_truck_menu->menu_category_id == $mc->id) 
                      ...
                      <div class="col-md-6">
                        ...
                        <div class="form-group">
                          <label for="status" class="content-label">{{trans('common.status')}}</label>
                            @if($food_truck_menu->status == 'active')  
                              <p class="details">{{trans('common.active')}}</p>
                            @endif
                            @if($food_truck_menu->status == 'inactive') 
                              <p class="details">{{trans('common.active')}}</p>
                            @endif
                        </div>
              ...
              <div class="modal-footer">
                <a class="btn btn-danger btn-fill btn-wd" href="{{route('food_truck_menus.edit',$food_truck_menu->id)}}">
                  {{trans('common.edit')}}
                </a>
              </div>
            </form>
