      ...
      <div class="col-xs-12">
        <div class="box">
          <div class="box-header">
            <h3 class="box-title">{{ trans('menu_categories.title') }}</h3>
            @can('menu-category-create')
            <h3 class="box-title pull-right"><a href="{{route('menu_categories.create')}}" class="btn btn-danger pull-right">{{trans('menu_categories.add_new')}}</a></h3>
            @endcan
          </div>
          <div class="box-body">
            <table id="menu_categories_table" class="table table-bordered table-hover">
              <thead>
                <tr>
                  <th>{{trans('common.id')}}</th>
                  <th>{{trans('menu_categories.name')}}</th>
...
<script type="text/javascript">
  $(document).on('change','.menu_category_status',function(){
      var status = $(this).val();
      var id = $(this).attr('id');
      var delay = 500;
      var element = $(this);
      $.ajax({
          type:'post',
          url: "{{route('menu_category_status')}}",
          data: {
                  "status": status, 
                  "id" : id,  
                  "_token": "{{ csrf_token() }}"
...
<script type="text/javascript">
  $(document).ready(function(){
    $('#menu_categories_table').DataTable({
      dom: 'Blfrtip',
      ...
      columns: [
         ...
         { data: 'status',
            mRender : function(data, type, row) {
                 ...
                 return '<select class="menu_category_status form-control" id="'+row["id"]+'"><option value="active"'+type+'>'+'{{trans("common.active")}}'+'</option><option value="inactive"'+data+'>'+'{{trans("common.inactive")}}'+'</option></select><span class="loading" style="visibility: hidden;"><i class="fa fa-spinner fa-spin fa-1x fa-fw"></i><span class="sr-only">Loading...</span></span>';
              } 
          },
         { 
            mRender : function(data, type, row) {
