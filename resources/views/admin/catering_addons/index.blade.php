...
@section('content')
  <section class="content-header">
    <h1>
      {{trans('catering_addons.plural')}}
    </h1>
    <ol class="breadcrumb">
      <li><a href="{{route('home')}}"><i class="fa fa-dashboard"></i> {{trans('common.home')}}</a></li>
      <li><a href="{{route('catering_addons.index')}}">{{trans('catering_addons.plural')}}</a></li>
    </ol>
  </section>
      ...
      <div class="col-xs-12">
        <div class="box">
          <div class="box-header">
            <h3 class="box-title">{{ trans('catering_addons.title') }}</h3>
            @can('catering-addon-create')
            <h3 class="box-title pull-right"><a href="{{route('catering_addons.create')}}" class="btn btn-danger pull-right">{{trans('catering_addons.add_new')}}</a></h3>
            @endcan
          </div>
          <div class="box-body">
              ...
              <thead>
                <tr>
                  ...
                  <th>{{trans('catering_addons.addon_name')}}</th>
                  <th>{{trans('catering_addons.addon_rate')}} {{$currency}}</th>
...
<script type="text/javascript">
  $(document).on('change','.catering_addon_status',function(){
      var status = $(this).val();
      var id = $(this).attr('id');
      var delay = 500;
      var element = $(this);
      $.ajax({
          type:'post',
          url: "{{route('catering_addon_status')}}",
          data: {
                  "status": status, 
                  "id" : id,  
                  "_token": "{{ csrf_token() }}"
        ...
        columns: [
           ...
           { data: 'status',
              mRender : function(data, type, row) {
                   ...
                   return '<select class="catering_addon_status form-control" id="'+row["id"]+'"><option value="active"'+type+'>'+'{{trans("common.active")}}'+'</option><option value="inactive"'+data+'>'+'{{trans("common.inactive")}}'+'</option></select><span class="loading" style="visibility: hidden;"><i class="fa fa-spinner fa-spin fa-1x fa-fw"></i><span class="sr-only">'+'{{trans("common.loading")}}'+'</span></span>';
                } 
            },
           { 
              mRender : function(data, type, row) {
