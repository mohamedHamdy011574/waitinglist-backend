...
@section('content')
  <section class="content-header">
    <h1>
      {{ trans('food_truck_branches.add_new') }}
    </h1>
    <ol class="breadcrumb">
      <li>
        <a href="{{route('home')}}">
      ...
      <div class="col-md-12">
        <div class="box">
          <div class="box-header with-border">
            ...
            @can('food-truck-list')
            <ul class="pull-right">
                <a href="{{route('food_truck_branches.index')}}" class="btn btn-danger">
          ...
          <div class="box-body">
            <form method="POST" id="message_templateForm" action="{{route('food_truck_branches.store')}}" accept-charset="UTF-8" enctype="multipart/form-data">
              @csrf
                ...
                <div class="row">
                  <div class="col-md-6">
                      ...
                      @foreach(config('app.locales') as $lk=>$lv)
                        <div role="tabpanel" class="tab-pane @if($lk=='en') active @endif" id="abc_{{$lk}}">
                          <div class="form-group">
                            ...
                            <input class="form-control"  placeholder="{{trans('food_truck_branches.name')}}" name="name:{{$lk}}" id="name:{{$lk}}" type="text" value="{{old('name:'.$lk)}}" required>
                            <strong class="help-block"></strong>
                          </div>
                          <div class="form-group">
                            <label for="address:{{$lk}}" class="content-label">{{trans('food_truck_branches.address')}}</label>
                            <textarea class="form-control" rows="5" placeholder="{{trans('food_truck_branches.address')}}" name="address:{{$lk}}" id="address:{{$lk}}" required>{{old('address:'.$lk)}}</textarea>
                            <strong class="help-block"></strong>
                          </div>
                        </div>    
                    ...
                    <div class="form-group">
                      ...
                      <select class="form-control select2" id="food_truck" name="food_truck_id" data-placeholder="{{trans('food_truck_branches.select_food_truck')}}" required>
                        <option value="">{{trans('food_truck_branches.select_food_truck')}}
                        </option>
                        @foreach($food_trucks as $food_truck)
                        <option value="{{$food_truck->id}}"
                          {{ ((old('food_truck_id') == $food_truck->id)) ? 'selected':'' }}
                          >{{$food_truck->name}}
                        </option>
                        @endforeach
                      </select>
                  ...
                  <div class="col-md-6">
                    ...
                    <div class="form-group">
                      ...
                      <select class="form-control select2" id="country" name="country_id" data-placeholder="{{trans('food_truck_branches.select_country')}}" required>
                        ...
                        <option value="{{$country->id}}"
                          {{ ((old('food_truck_id') == $country->id)) ? 'selected':'' }}
                    ...
                    <div class="form-group">
                      ...
                      <select class="form-control select2" id="state" name="state_id" data-placeholder="{{trans('food_truck_branches.select_state')}}" required>
                        <option value="">{{trans('food_truck_branches.select_state')}}
                        </option>
