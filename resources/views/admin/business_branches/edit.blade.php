              ...
              <div class="model-body">
                      ...
                      @foreach(config('app.locales') as $lk=>$lv)
                        <div role="tabpanel" class="tab-pane @if($lk=='en') active @endif" id="abc_{{$lk}}">
                          <div class="form-group">
                            ...
                            <input class="form-control"  placeholder="{{trans('business_branches.branch_name')}}" name="branch_name:{{$lk}}" id="branch_name:{{$lk}}" type="text" value="{{$business_branch->translate($lk)->branch_name}}"
                  ...
                  <div class="col-md-6">
                    <div class="form-group">
                      ...
                      <select class="form-control" id="branch_type" name="branch_type">
                        ...
                        {{ ($business_branch->branch_type == 'restaurant') ? 'selected':'' }}
                        >{{trans('business_branches.restaurant')}}</option>
                        @endif

                        @if($subscription->for_catering && $catering_option == 1)
                        <option value="catering" 
                        {{ ($business_branch->branch_type == 'catering') ? 'selected':'' }}>{{trans('business_branches.catering')}}</option>
                        @endif

                        @if($subscription->for_food_truck)
                        <option value="food_truck" {{ ($business_branch->branch_type == 'food_truck') ? 'selected':'' }}>{{trans('business_branches.food_truck')}}</option>
                        @endif
                      ...
                      <div class="row">
                        <div class="col-md-4">
                          <div class="form-group">
                            <label for="min_notice" class="content-label">{{trans('business_branches.min_notice')}}</label>
                              <input type="number" class="form-control" id="min_notice" @if($business_branch->branch_type == 'catering') min="1" @endif name="min_notice" value="{{$business_branch->min_notice}}"> 
                        ...
                        <div class="col-md-4">
                          <div class="form-group">
                            ...
                            <input type="number" class="form-control" id="min_notice" @if($business_branch->branch_type == 'catering') min="1" @endif name="min_order" value="{{$business_branch->min_order}}"> 
                            <strong class="help-block"></strong>
                          </div>
                        </div>
                        <div class="col-md-4">
                ...
                <div id="map"></div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control" name="address" id="address_autocom">{{$business_branch->address}}</textarea>
                        <strong class="help-block"></strong>
                    </div>
                    <div class="form-group">
                  ...
                  <div class="col-md-6">
                    ...
                    <div class="form-group">
                        <label for="total_seats" class="content-label">{{trans('business_branches.branch_email')}}</label>
                          <input class="form-control"  placeholder="{{trans('business_branches.branch_email')}}" name="branch_email" id="branch_email" type="email" value="{{$business_branch->branch_email}}" required>
                        <strong class="help-block"></strong>
                    </div>
                    <div class="form-group">
                        <label for="total_seats" class="content-label">{{trans('business_branches.branch_phone_number')}}</label>
                          <input class="form-control"  placeholder="{{trans('business_branches.branch_phone_number')}}" name="branch_phone_number" min="1" id="branch_phone_number" type="number" value="{{$business_branch->branch_phone_number}}" required>
                        <strong class="help-block"></strong>
                    </div>
                    ...
                    <div class="form-group" id="seating_area_div">
                      ...
                      <select class="form-control select2" id="seating_area" name="stg_area_id[]" multiple data-placeholder="{{trans('seating_area.select_seating_area')}}">
                        ...
                        <option value="{{$seating_area->id}}"
                          {{ (collect($selected_seating_areas)->contains($seating_area->id)) ? 'selected':'' }}
                          >{{$seating_area->name}}
                        </option>
                        @endforeach
                      </select>
                    ...
                    <div class="form-group">
                      ...
                      <select class="form-control" name="status" id="status" required>
                        <option value="active" 
                          @if($business_branch->status == 'active') selected @endif>
                          {{trans('common.active')}}
                        </option>
                        <option value="inactive" 
                          @if($business_branch->status == 'inactive') selected @endif>
...
<script>
  $( document ).ready(function() {
    ...
    setTimeout(function(){ 
      ...
      $("#state option[value='{{$business_branch->state_id}}']").prop('selected', true);
    }, 3000);
  });
