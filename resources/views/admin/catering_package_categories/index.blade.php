...
@section('content')
  <section class="content-header">
    <h1>
      {{trans('catering_package_categories.plural')}}
    </h1>
    <ol class="breadcrumb">
      <li><a href="{{route('home')}}"><i class="fa fa-dashboard"></i> {{trans('common.home')}}</a></li>
      <li><a href="{{route('catering_package_categories.index')}}">{{trans('catering_package_categories.plural')}}</a></li>
    </ol>
  </section>
      ...
      <div class="col-xs-12">
        <div class="box">
          <div class="box-header">
            <h3 class="box-title">{{ trans('catering_package_categories.title') }}</h3>
            @can('catering-package-category-create')
            <h3 class="box-title pull-right"><a href="{{route('catering_package_categories.create')}}" class="btn btn-danger pull-right">{{trans('catering_package_categories.add_new')}}</a></h3>
            @endcan
          </div>
          <div class="box-body">
              ...
              <thead>
                <tr>
                  ...
                  <th>{{trans('catering_package_categories.name')}}</th>
...
<script type="text/javascript">
  $(document).on('change','.catering_package_categories_status',function(){
      var status = $(this).val();
      var id = $(this).attr('id');
      var delay = 500;
      var element = $(this);
      $.ajax({
          type:'post',
          url: "{{route('catering_package_categories_status')}}",
          data: {
                  "status": status, 
                  "id" : id,  
                  "_token": "{{ csrf_token() }}"
  ...
  $(document).ready(function(){
    ...
    function fill_datatable() {
      $('#catering_table').DataTable({
        ...
        ajax: {
            url: "{{route('dt_catering_package_categories')}}",
        ...
        columns: [
           ...
           { data: 'status',
              mRender : function(data, type, row) {
                   ...
                   return '<select class="catering_package_categories_status form-control" id="'+row["id"]+'"><option value="active"'+type+'>'+'{{trans("common.active")}}'+'</option><option value="inactive"'+data+'>'+'{{trans("common.inactive")}}'+'</option></select><span class="loading" style="visibility: hidden;"><i class="fa fa-spinner fa-spin fa-1x fa-fw"></i><span class="sr-only">'+'{{trans("common.loading")}}'+'</span></span>';
                } 
            },
           { 
              mRender : function(data, type, row) {
