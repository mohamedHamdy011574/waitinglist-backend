...
@section('content')
  <section class="content-header">
    <h1>
      {{trans('pickup_hours.plural')}}
    </h1>
    <ol class="breadcrumb">
      <li><a href="{{route('home')}}"><i class="fa fa-dashboard"></i> {{trans('common.home')}}</a></li>
      <li><a href="{{route('pickup_hours.index')}}">{{trans('pickup_hours.plural')}}</a></li>
    </ol>
  </section>
      ...
      <div class="col-xs-12">
        <div class="box">
          <div class="box-header">
            <h3 class="box-title">{{ trans('pickup_hours.title') }}</h3>
            @can('pickup-hours-create')
            <h3 class="box-title pull-right"><a href="{{route('pickup_hours.create')}}" class="btn btn-danger pull-right">{{trans('pickup_hours.add_new')}}</a></h3>
            @endcan
          </div>
          <div class="box-body">
            <table id="pickup_hours_table" class="table table-bordered table-hover">
              <thead>
                <tr>
                  <th>{{trans('common.id')}}</th>
                  <th>{{trans('pickup_hours.shifts')}}</th>
                  <th>{{trans('pickup_hours.from_time')}}</th>
                  <th>{{trans('pickup_hours.to_time')}}</th>
...
<script type="text/javascript">
  $(document).on('change','.pickup_hours_status',function(){
      var status = $(this).val();
      var id = $(this).attr('id');
      var delay = 500;
      var element = $(this);
      ...
      columns: [
         ...
         { data: 'status',
            mRender : function(data, type, row) {
                 ...
                 return '<select class="pickup_hours_status form-control" id="'+row["id"]+'"><option value="active"'+type+'>'+'{{trans("common.active")}}'+'</option><option value="inactive"'+data+'>'+'{{trans("common.inactive")}}'+'</option></select><span class="loading" style="visibility: hidden;"><i class="fa fa-spinner fa-spin fa-1x fa-fw"></i><span class="sr-only">'+'{{trans("common.inactive")}}'+'</span></span>';
              } 
          },
         { 
            mRender : function(data, type, row) {
                 return '@can("pickup-hours-edit")<a class="btn" href="'+row["edit"]+'"><i class="fa fa-edit"></i></a>@endcan<a class="btn" href="'+row["show"]+'"><i class="fa fa-eye"></i></a>@can("pickup-hours-delete")<form action="'+row["delete"]+'" method="post"><button class="btn" type="submit" onclick=" return delete_alert()"><i class="fa fa-trash"></i></button>@method("delete")@csrf</form>@endcan';  
